...
while($rowdata = mysqli_fetch_assoc($sql)){
	$old_total_amount = $rowdata['total_amount'];
	$old_total_due = $rowdata['total_due'];
	
	$new_total_amount  = $old_total_amount+$pay_cost;
	$new_total_due  = $old_total_due+$pay_cost;
}
$sql1 = mysqli_query($con,"UPDATE `supplier_payment_log` SET `total_amount`='$new_total_amount',`total_due`='$new_total_due' WHERE `pdc_bayer_id`='$customer_details'");



		$bill_no = fetchLastBill()+1;

function insertProduct($customer_details,$order_no,$date,$so_no,&$product_name,$invoice_no,$place_of_supply,&$serial_no,$mat_code,&$mat_des,&$hsn_code,$freight,&$cgst,&$sgst,&$igst,$customer_drg_no,&$quantity,&$rate,&$discount,&$amount,&$tax_cost,$pay_cost,$bill_no,&$quantity_avail,&$imeino,&$color){
	global $con;
	
	$sql = mysqli_query($con,"INSERT INTO `insertproduct`(`id`, `customer_details`, `order_no`, `rdate`, `so_no`,product_name, `invoice_no`, `place_of_supply`, `sl_no`, `mat_code`, `mat_des`, `hsn_code`, `freight`, `cgst`, `sgst`, `igst`, `customer_drg_no`, `quantity`, `rate`, `discount`, `amount`, `finance_amount`, `tax_cost`, `pay_cost`, `bill`, `quantity_avail`, `imeino`, `color`) VALUES ('','$customer_details','$order_no','$date','$so_no',$product_name,'$invoice_no','$place_of_supply','$serial_no','$mat_code','$mat_des','$hsn_code','$freight','$cgst','$sgst','$igst','$customer_drg_no','$quantity','$rate','$discount','$amount','','$tax_cost','$pay_cost','$bill_no','$quantity_avail','$imeino','$color')");
...
//echo($mat_code[1]);
	for($i = 0; $i<count($serial_no); $i++) {
		...
		//echo($mat_code);
		$sql = insertProduct($customer_details,$order_no,$date,$so_no,$product_name[$i],$invoice_no,$place_of_supply,$serial_no[$i],$mat_code,$mat_des[$i],$hsn_code[$i],$freight,$cgst[$i],$sgst[$i],$igst,$customer_drg_no,
